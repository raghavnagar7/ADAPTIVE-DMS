...
def load_recent_session(
    ...
    try:
        ...
        numeric_columns = [
            ...
            # GRU temporal predictor fields
            "gru_risk",
            "gru_confidence",
            "gru_sequence_progress",
...
def get_gru_column(df, aliases, default=None):
    """Return the first available GRU column from a list of aliases."""
    for name in aliases:
...
def classify_gru_prediction(value):
    """Normalize GRU classification labels for dashboard display."""
    text = safe_string(value, "UNKNOWN").strip().upper()
    mapping = {
        "0": "NORMAL",
        "1": "FATIGUE",
        "2": "DROWSY",
...
def prepare_dataframe(
    ...
    # ---------------------------------------------------------
    # GRU temporal predictor compatibility
    # ---------------------------------------------------------
    gru_risk_col = get_gru_column(
        df,
        [
            "gru_risk",
            "gru_fatigue_risk",
    ...
    if gru_risk_col is None:
        df["gru_risk"] = float("nan")
    elif gru_risk_col != "gru_risk":
        df["gru_risk"] = pd.to_numeric(
            df[gru_risk_col], errors="coerce"
        )

    gru_conf_col = get_gru_column(
        df,
        [
            "gru_confidence",
            "temporal_gru_confidence",
            "gru_prediction_confidence",
        ],
    )
    if gru_conf_col is None:
        df["gru_confidence"] = float("nan")
    elif gru_conf_col != "gru_confidence":
        df["gru_confidence"] = pd.to_numeric(
            df[gru_conf_col], errors="coerce"
        )

    gru_class_col = get_gru_column(
        df,
        [
            "gru_classification",
            "gru_class",
            "gru_prediction",
            "temporal_gru_classification",
        ],
    )
    if gru_class_col is None:
        df["gru_classification"] = "UNKNOWN"
    elif gru_class_col != "gru_classification":
        df["gru_classification"] = df[gru_class_col].apply(
            classify_gru_prediction
        )
    else:
        df["gru_classification"] = df["gru_classification"].apply(
            classify_gru_prediction
        )

    progress_col = get_gru_column(
        df,
        [
            "gru_sequence_progress",
            "sequence_progress",
    ...
    if progress_col is None:
        df["gru_sequence_progress"] = float("nan")
    elif progress_col != "gru_sequence_progress":
        df["gru_sequence_progress"] = pd.to_numeric(
            df[progress_col], errors="coerce"
        )

    status_col = get_gru_column(
        df,
        [
            "gru_model_status",
            "gru_status",
    ...
    if status_col is None:
        df["gru_model_status"] = "NOT_RECORDED"
    elif status_col != "gru_model_status":
        df["gru_model_status"] = df[status_col].astype(str)
...
def calculate_session_analytics(
    ...
    # ---------------------------------------------------------
    # GRU temporal predictor analytics
    # ---------------------------------------------------------
    gru_risk = pd.to_numeric(
        df.get("gru_risk", pd.Series(dtype=float)),
        errors="coerce",
    )
    gru_conf = pd.to_numeric(
        df.get("gru_confidence", pd.Series(dtype=float)),
        errors="coerce",
    )

    valid_gru_risk = gru_risk.dropna()
    valid_gru_conf = gru_conf.dropna()

    result["gru_sample_count"] = int(len(valid_gru_risk))
    result["gru_average_risk"] = (
        float(valid_gru_risk.mean()) if not valid_gru_risk.empty else 0.0
    )
    result["gru_maximum_risk"] = (
        float(valid_gru_risk.max()) if not valid_gru_risk.empty else 0.0
    )
    result["gru_average_confidence"] = (
        float(valid_gru_conf.mean()) if not valid_gru_conf.empty else 0.0
    )

    if "gru_classification" in df.columns:
        classes = (
            df["gru_classification"]
            .astype(str)
            .str.upper()
            .replace("NAN", "UNKNOWN")
        )
        result["gru_latest_classification"] = (
            classes.iloc[-1] if len(classes) else "UNKNOWN"
        )
        result["gru_class_distribution"] = classes.value_counts().to_dict()
    else:
        result["gru_latest_classification"] = "UNKNOWN"
        result["gru_class_distribution"] = {}

    if "gru_sequence_progress" in df.columns:
        progress = pd.to_numeric(
            df["gru_sequence_progress"], errors="coerce"
        ).dropna()
        result["gru_sequence_progress"] = (
            float(progress.iloc[-1]) if not progress.empty else 0.0
        )
    else:
        result["gru_sequence_progress"] = 0.0

    if "gru_model_status" in df.columns:
        result["gru_model_status"] = safe_string(
            df["gru_model_status"].iloc[-1], "NOT_RECORDED"
        ).upper()
    else:
        result["gru_model_status"] = "NOT_RECORDED"
...
def generate_report(
    ...
    current_gru_risk=0.0,
    current_gru_confidence=0.0,
    current_gru_classification="UNKNOWN",
    current_gru_progress=0.0,
    current_gru_status="NOT_RECORDED",
):
    """
    ...
    lines = [
        ...
        f"Current GRU Risk: {current_gru_risk:.3f}",
        f"GRU Confidence: {current_gru_confidence:.3f}",
        f"GRU Classification: {current_gru_classification}",
        f"Sequence Progress: {current_gru_progress:.0f}/20",
        f"GRU Model Status: {current_gru_status}",
        f"Average GRU Risk: {analytics.get('gru_average_risk', 0):.3f}",
        f"Average GRU Confidence: {analytics.get('gru_average_confidence', 0):.3f}",
...
gru_risk = safe_float(latest.get("gru_risk", float("nan")), 0.0)
gru_confidence = safe_float(latest.get("gru_confidence", float("nan")), 0.0)
gru_classification = classify_gru_prediction(
    latest.get("gru_classification", "UNKNOWN")
)
gru_progress = safe_float(
    latest.get("gru_sequence_progress", float("nan")), 0.0
)
gru_model_status = safe_string(
    latest.get("gru_model_status", "NOT_RECORDED"),
    "NOT_RECORDED",
).upper()
...
st.caption(
    "Temporal fatigue prediction from the 20-sample GRU sequence."
)

c1, c2, c3, c4, c5 = st.columns(5)

with c1:
    st.metric("GRU Risk", f"{gru_risk:.3f}")

with c2:
    st.metric("GRU Confidence", f"{gru_confidence:.3f}")

with c3:
    st.metric("GRU Class", gru_classification)

with c4:
    progress_text = f"{int(gru_progress)}/20" if gru_progress <= 20 else f"{gru_progress:.0f}"
    st.metric("Sequence", progress_text)

with c5:
    st.metric("Model", gru_model_status)

if gru_model_status in {"LOADED", "ACTIVE", "READY"}:
    st.success("🟢 GRU temporal model is active.")
elif gru_model_status in {"FALLBACK", "ERROR", "FAILED"}:
    st.warning("🟡 GRU is using fallback/error state recorded by main.py.")
else:
    st.info("ℹ️ GRU status has not been recorded in the session CSV.")
...
# =============================================================
# GRU RISK VS FUSION RISK
# =============================================================

st.subheader(
    "🧠 Fusion Risk vs GRU Risk"
)
...
if gru_plot.empty:
    st.info(
        "No GRU predictions are available in the session CSV yet. "
        "Run main.py until the 20-sample GRU sequence is populated."
    )
else:
    fig_gru = go.Figure()
    ...
    fig_gru.add_trace(
        go.Scatter(
            x=gru_plot["elapsed_seconds"],
            y=gru_plot["fatigue_risk"],
            mode="lines",
            name="Fusion Risk",
        )
    )
...
st.subheader(
    "🎯 GRU Prediction Confidence"
)

confidence_plot = df.dropna(
    subset=["gru_confidence"]
).copy()

if confidence_plot.empty:
    st.info("No GRU confidence values are available yet.")
else:
    fig_gru_conf = go.Figure()
    fig_gru_conf.add_trace(
        go.Scatter(
            x=confidence_plot["elapsed_seconds"],
            y=confidence_plot["gru_confidence"],
            mode="lines",
            name="GRU Confidence",
        )
    )
    fig_gru_conf.update_layout(
        xaxis_title="Time (seconds)",
...
report_text = generate_report(
    ...
    current_gru_risk=gru_risk,
    current_gru_confidence=gru_confidence,
    current_gru_classification=gru_classification,
    current_gru_progress=gru_progress,
    current_gru_status=gru_model_status,
)
...
report_rows = [
    ...
    {
        "metric": "gru_average_risk",
        "value": analytics["gru_average_risk"],
    },

    {
        "metric": "gru_maximum_risk",
        "value": analytics["gru_maximum_risk"],
    },

    {
        "metric": "gru_average_confidence",
        "value": analytics["gru_average_confidence"],
    },

    {
        "metric": "gru_sample_count",
        "value": analytics["gru_sample_count"],
    },
